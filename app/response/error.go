...
func HandleErrors(c *fiber.Ctx, e error) error {
	...
	// 	return NewError(c, fiber.StatusBadRequest, token.ErrExpiredToken.Error(), nil)
	// }

	// // token invalid 401
	// if errors.Is(e, token.ErrInvalidToken) {
	// 	return NewError(c, fiber.StatusUnauthorized, token.ErrInvalidToken.Error(), nil)
	// }

	// no record in table of database | 404
	if errors.Is(e, sql.ErrNoRows) {
		return NewError(c, fiber.StatusNotFound, ErrNotFound.Error(), nil)
	}
